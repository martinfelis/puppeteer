...
--// matrix.totable ( m1 )
-- creates a flat lua table from a row or column vector
function matrix.totable ( m1 )
	local result = {}
	if m1:rows() == 1 then
		for i,v in ipairs (m1[1]) do
			table.insert (result, v)
		end
	elseif m1:columns() == 1 then
		for i = 1,m1:rows() do
			table.insert (result, m1[i][1])
		end
	else
		error ("matrix:totable() only allowed for column or row vectors")
	end
	return result
end
