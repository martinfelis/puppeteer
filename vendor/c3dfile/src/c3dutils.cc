...
void print_c3dheaderinfo (const C3DHeader &header) {
	if (header.c3d_id != 80) {
		cout << "Invalid C3D id at byte 2! Is: 0x" << hex << static_cast<int>(header.c3d_id)
			<< " but should be: 0x50" << endl;
...
void print_parameter_info (const ParameterInfo &parameter_info) {
	...
	for (i = 0; i < parameter_info.n_dimensions; i++) {
		cout << "  " << "dim[" << i << "]: " << static_cast<int>(parameter_info.dimensions[i]) << endl;
	}
	
	if (parameter_info.data_type == 1 || parameter_info.data_type == -1)
		cout << "  " << "val   : " << parameter_info.char_data[0] << endl;
	if (parameter_info.data_type == 2)
		cout << "  " << "val   : " << static_cast<Sint16>(parameter_info.int_data[0]) << endl;
	if (parameter_info.data_type == 4)
		cout << "  " << "val   : " << parameter_info.float_data[0] << endl;

	cout	<< "  " << "desc  : " << parameter_info.description 
